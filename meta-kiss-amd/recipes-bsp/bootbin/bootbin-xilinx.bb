...
# Returns a dedicated TMPDIR path for microblaze architecture builds.
def get_microblaze_tmpdir(d):
    tmpdir = d.getVar('TMPDIR')
    tclibcappend = d.getVar('TCLIBCAPPEND')
    if tclibcappend:
        return tmpdir.replace(tclibcappend, '-newlib_microblaze')
    else:
        return tmpdir + '_microblaze'
